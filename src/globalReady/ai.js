...
// AICoach.jsx and Career Ready already use elsewhere in HSDOS.AI.
...
// beginners can start immediately without already knowing English. Unlike
// Career Ready (one persona per category), Global Ready roleplays a
// different character per scenario — homestay parent, immigration officer,
// classmate, etc. — so the persona is passed in per scenario.
export function buildConversationSystemPrompt(persona, memorySummary) {
  return `You are a warm, confident English conversation coach inside Global Ready, a voice-first pathway of HSDOS.AI for Japanese university students preparing for study abroad, travel, and international life. Confidence before correctness — never make the student feel bad about mistakes.
...
- Help the student sound natural, not textbook — real conversational English, not stiff recited phrases.
...
export function buildOpeningUserMessage(scenarioPrompt) {
  return `Session start. Greet the student warmly in character, then naturally set up this situation: "${scenarioPrompt}". Keep it short and welcoming.`;
}
...
export async function askCoach(system, messages, user) {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.error ?? "Global Ready coach is unavailable right now.");
  }
