...
// subtitle underneath so beginners can start before they're fluent.
export default function PracticeSession({ categoryId, user, onExit, onBadgesEarned }) {
  const category = CATEGORY_MAP[categoryId];
  const questions = PRACTICE_BANKS[categoryId];
  const { lang } = useLang();
  const tr = (k) => crt(lang, k);
  const jp = lang === "jp";

  const [index, setIndex]         = useState(0);
  const [messages, setMessages]   = useState([]); // [{role:"user"|"assistant", text, en, jp}]
  ...
  async function openTopic(idx) {
    ...
    setLoading(true);
    try {
      const system = buildConversationSystemPrompt(categoryId, memoryRef.current.summary);
      const raw = await askCoach(system, [{ role: "user", content: buildOpeningUserMessage(questions[idx].prompt) }], user);
      const parsed = parseConversationReply(raw);
      setMessages([{ role: "assistant", en: parsed.reply_en, jp: parsed.reply_jp }]);
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "24px 16px 40px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        ...
        <button
          onClick={() => !loading && !isLast && openTopic(index + 1)}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: COLORS.textMuted }}>
            {tr("topic_of")} {index + 1} {tr("of_word")} {questions.length} · {jp && question.promptJp ? question.promptJp : question.prompt}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
        {questions.map((q, i) => (
          <div key={q.id} style={{ flex: 1, height: 4, borderRadius: 2, background: i < index ? category.color : i === index ? `${category.color}88` : "#222" }} />
        ))}
      </div>
      ...
      <div style={{
        ...
        minHeight: 320, maxHeight: 440, overflowY: "auto", marginBottom: 16,
      }}>
